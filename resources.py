...
class UserListResource(Resource):
    ...
    @marshal_with(user_fields)
    def post(self): ## TODO if email already exists dont't try to commit
...
class RecordingListResource(Resource):
    ...
    @marshal_with(recording_fields) ## TODO if s3 url already exists dont't try to commit
    def post(self):
...
class PermissionResource(Resource):
    ...
    def put(self, user_id, recording_id):
        ...
        if (not queried_user) or (not queried_recording):
            abort(404, message='User/recording doest not exist')
